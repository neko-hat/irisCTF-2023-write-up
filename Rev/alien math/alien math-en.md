...
Return the random value between the two factors with `bpafbz(inta1, inta2)` -> `return (unsigned int) (rand() % (a2 - a1) + a1)` inside `ajjgpfl(inta1)`.
...
After that, the character string is reversed, and the character string is used as the actual index of `lfwp` and stored in `v11` assigned to the heap.

`lfwp` is initialized as follows.
